...
docker exec -i kafka bash -c 'opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server kafka:9092 --delete --topic historical-raw-events ; sleep 5; opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server kafka:9092 --create --topic historical-raw-events --partitions 4 --replication-factor 1'
